...
from stegid.receipts import verify_receipt  # StegID verifier (v0)
...
# ---------------------------
# StegID -> VerifiedReceipt loader (env injected by workflow)
# ---------------------------
...
def _load_pubkeys() -> Dict[str, str]:
    """
    Where StegID public keys are stored (kid -> public_b64).
    Prefer local file in this repo if you mirror StegID public keys here.
    """
    # Option A: keep a copy in this repo at public_keys/keys.json
    p = REPO_ROOT / "public_keys" / "keys.json"
    if p.exists():
        return json.loads(p.read_text(encoding="utf-8"))
    # Option B: allow direct env injection for quick wiring
...
def _as_stegcore_verified_receipt(receipt: dict) -> dict:
    """
    StegCore expects a VerifiedReceipt-like dict with at least:
    - actor_class
    - scopes
    - issued_at / expires_at (ISO)
    - assurance_level
    - signals
    plus the proof fields (kid/sig/payload_hash) if present.

    We pass the receipt through as dict. StegCore's decide() should treat
    missing/invalid receipts as DENY/DEFER.
    """
...
def _gate_agent_or_raise(agent_name: str) -> None:
    """
    Single choke-point for ALL agents.
    - If no receipt: DENY (safe default)
    - If receipt invalid: DENY (safe default)
    - If StegCore does not ALLOW: DENY/DEFER
    """
...
def run_agent(agent_name: str) -> None:
    ...
    Adds StegCore/StegID bounding as a choke point.
    """
